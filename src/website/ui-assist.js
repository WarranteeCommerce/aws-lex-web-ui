...
function handleIncomingMsg(msg){
    ...
    if(isBot){
        ...
        msg.mad = msg.innerText.length * 10;
        msg.style.animationDuration = msg.mad+"ms";
        let time = window.botMsgStack[0] && window.botMsgStack[0].mad;
        setTimeout(function(){
            msg.classList.add('show');
            scrollToBottomOfMsgContainer();
    ...
    // todo if user on mobile
    if(window.isMobile){
        let input = document.getElementById('text-input')
        input.onfocus = ()=>{input.blur(), input.onfocus = null};
    }
}
...
if(window.isMobile){
    ...
    window.addEventListener('resize', function(){
        ...
        timeout = setTimeout(doOnResize,100);
    })
}

// initializing mutation observer to manage messages
